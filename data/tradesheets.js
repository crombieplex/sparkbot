...
const { clientId, clientSecret, redirectUri, spreadsheetId, spreadsheetTabNameTrade, ownerId } = require('../config.json')
const fs = require("fs")
...
const sendUpdate = () => {
    ...
    authorize((err, auth) => {
        ...
        sheets.spreadsheets.values.update({
            ...
            range: `${spreadsheetTabNameTrade}!A:I`,
            ...
            if (err) {
                logger.error('Failed to update tradeskills: ' + err);
...
const buildValueArray = (obj) => {
    let value = [["Discord ID", "Name", "Weaponsmithing", "Armoring", "Engineering", "Jewelcrafting", "Arcana", "Cooking", "Furnishing", "Notes"]]

    let entries = Object.entries(obj).sort((a, b) => {
        return a[1].name.localeCompare(b[1].name);
    })
    for (const [k, v] of entries) {
        value.push([k, v.name, v.weaponsmithing || " ", v.armoring || " ", v.engineering || " ", v.jewelcrafting || " ", v.arcana || " ", v.cooking || " ", v.furnishing || " ", v.notes || " "])
    }
    return value
}
