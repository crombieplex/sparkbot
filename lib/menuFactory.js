...
module.exports.getMainCharacterMenu = (user, character) => {
    let embed = new MessageEmbed()
    ...
    if (user.grants.company) {
        row.addComponents(new MessageButton().setCustomId('mainMenuManageGrantsBtn').setLabel('Manage Grants').setStyle('PRIMARY'))
    }
