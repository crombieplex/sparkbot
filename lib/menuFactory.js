...
module.exports.getMainCharacterMenu = (user) => {
    ...
    if (user.grants.company) {
        let companyRow = new MessageActionRow()
            .addComponents(new MessageButton().setCustomId('mainMenuManageTradeBtn').setLabel('Trade Skills').setStyle('DANGER'))
        components.push(companyRow)
    }
    return { embeds: [embed], components: components }
}
